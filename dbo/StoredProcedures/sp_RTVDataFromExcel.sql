...
-- exec sp_RTVDataFromExcel '207','Heavy Duty Truck',8009,'Attenuator Trucks','GMC',1
-- =============================================
CREATE PROCEDURE sp_RTVDataFromExcel  
  @CategoryId int,
...
BEGIN
 ...
 set @CatId = (Select Id from EquipmentCategory_V2 ec where lower(ec.Name) = lower(@CategoryName) and IsDeleted=0 and SiteId=@SiteId and ec.SiteCategoryId=@CategoryId)
	...
	begin
		Insert into EquipmentCategory_V2 values (@CategoryName,@SiteId,@CategoryName,@CategoryId,getdate(),'By Procedure',getdate(),'By Procedure',0)
 ...
 set @SubCatId = (Select Id from EquipmentSubCategory_V2 esc where LOWER(esc.Name) = lower(@SubcategoryName) and IsDeleted=0 and esc.EquipmentCategoryId=@CatId and esc.SiteSubCategoryId=@SubcategoryId)
	...
	begin
		insert into EquipmentSubCategory_V2 values (@SubcategoryName,@CatId,@SubcategoryName,@SubcategoryId,getdate(),'By Procedure',getdate(),'By Procedure',0)
