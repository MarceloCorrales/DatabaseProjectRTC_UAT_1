...
CREATE PROCEDURE [dbo].[sp_CleanseDataGapsEquipment]
AS
...
/** Fill in data gaps in years for each make and model **/
     ...
     FROM [dbo].[MachineryTraderEquipment] t
		...
		JOIN [dbo].[Site] s ON c.SiteId = s.Id
   WHERE S.[Name] = 'https://www.machinerytrader.com/'
     ...
     IF OBJECT_ID('dbo._tmp2', 'U') IS NOT NULL
         DROP TABLE dbo._tmp2;
     SELECT A.*,
            CASE
