...
CREATE PROCEDURE [dbo].[sp_CleanseDataGapsMediumTrucks]
AS
	...
	--3500HD	1998
     ...
     FOR
         SELECT DISTINCT
                [MAKE],
                [MODEL]
         FROM [dbo].[TruckPaperMedEquipmentYearGaps] a
         WHERE EXISTS
         (
             SELECT *
             FROM [dbo].[TruckPaperMedEquipmentYearGaps] b
             WHERE a.make = b.make
                   AND a.model = b.model
         ...
         (
             ...
             (
                 SELECT [MAKE],
                        [MODEL]
                 FROM [dbo].[TruckPaperMedEquipmentYearGaps]
                 GROUP BY [MAKE],
                          [MODEL]
                 HAVING COUNT(1) = 1
             ) c
