...
--select top 6 * from EquipmentFutureValues

--exec  [dbo].[CreateAllEquipmentParentOne]

CREATE procedure [dbo].[CreateAllEquipmentParentOne]
as
...
@model1 varchar(200), 
 ...
 TRUNCATE TABLE EquipmentFutureValues1
...
SELECT e.name, ParentModel model , c.id CategoryId, s.id SubcategoryId,  year
FROM [dbo].[MachineryTraderEquipment] m  (nolock)
	...
	WHERE year <> 0 and [HoursMiles] <> 0 AND  ParentModel = N'D6T LGP' -- and c.id = 1025 and s.id = 1026
	group by e.name,  ParentModel, year, c.id, s.id, c.name , s.name 
	ORDER BY 1,2,5
...
BEGIN  
  ...
  EXEC  [dbo].[sp_CalculateEquipmentFutureValues_DEP_EQ_One] 
	     @Make = @make1,
		@Model = @model1 ,
