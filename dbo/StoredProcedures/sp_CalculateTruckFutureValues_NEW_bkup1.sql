...
CREATE  PROCEDURE [dbo].[sp_CalculateTruckFutureValues_NEW_bkup1]
(
 @Make                VARCHAR(100), 
 @Model               VARCHAR(50), 
 @YearMake            INT, 
...
IF  ((@SelectedIds IS NULL OR LTRIM(RTRIM(@SelectedIds)) = '' ) AND (@MilesHoursAdjusted IS NULL))
BEGIN
INSERT @T
 (Make
,Model
...
,LogSalesPrice
 ...
 --    AND m.id not in (SELECT * FROM  dbo.Split(@SelectedIds, ','))
   ...
   [YEAR]
END
ELSE IF (@SelectedIds IS NOT NULL)
BEGIN
INSERT @T
 (Make
,Model
...
SELECT
 ...
 GROUP BY 
   ...
   [YEAR]
END
ELSE IF (@MilesHoursAdjusted IS NOT NULL)
BEGIN
INSERT @T
 (Make
,Model
